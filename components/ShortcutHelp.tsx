...
export const ShortcutHelp: React.FC<ShortcutHelpProps> = ({ isOpen, onClose }) => {
  ...
  const SHORTCUTS = [
    { category: 'Editing', items: [
        { keys: [modifierSymbol, 'C'], label: 'Copy' },
        { keys: [modifierSymbol, 'X'], label: 'Cut' },
        { keys: [modifierSymbol, 'V'], label: 'Paste' },
        { keys: [modifierSymbol, 'D'], label: 'Duplicate' },
        { keys: ['Del'], label: 'Delete' },
        { keys: [modifierSymbol, 'Z'], label: 'Undo' },
        { keys: [modifierSymbol, 'Shift', 'Z'], label: 'Redo' },
    ]},
    { category: 'Navigation', items: [
        { keys: [modifierSymbol, '+'], label: 'Zoom In' },
        { keys: [modifierSymbol, '-'], label: 'Zoom Out' },
        { keys: [modifierSymbol, '0'], label: 'Reset Zoom' },
        { keys: ['Space', 'Drag'], label: 'Pan Canvas' },
        { keys: [modifierSymbol, 'F'], label: 'Search / AI' },
    ]},
    { category: 'Creation', items: [
        { keys: [modifierSymbol, 'N'], label: 'New Node' },
        { keys: ['Dbl Click'], label: 'Edit Node' },
    ]},
    { category: 'Selection', items: [
        { keys: [modifierSymbol, 'A'], label: 'Select All' },
      ...
      <div className="bg-[#181B21] border border-[#2D313A] rounded-2xl w-full max-w-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-t border-[#2D313A] bg-[#181B21] text-center">
            <p className="text-xs text-gray-500">
                Press <kbd className="px-1.5 py-0.5 bg-[#0F1115] border border-[#2D313A] rounded text-[10px] mx-1">{modifierSymbol}</kbd> + <kbd className="px-1.5 py-0.5 bg-[#0F1115] border border-[#2D313A] rounded text-[10px] mx-1">/</kbd> to toggle this modal
            </p>
        </div>
      </div>
    </div>
