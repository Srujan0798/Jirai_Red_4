...
const GraphEngineInner: React.FC<GraphEngineProps> = ({ viewMode }) => {
  ...
  // Fit view once on initial load or reset
  useEffect(() => {
    const timer = setTimeout(() => fitView({ duration: 800 }), 100);
    return () => clearTimeout(timer);
  }, [fitView, nodes.length === 3]); // Re-run if we reset to initial state
  ...
  return (
    ...
    <div id="main-canvas" className="w-full h-full bg-[#0F1115] relative overflow-hidden pb-20" ref={reactFlowWrapper}>
      ...
      >
        ...
        <Panel position="bottom-center" className="mb-4 pointer-events-none opacity-50">
        ...
        <Controls position="bottom-left" className="bg-[#181B21] border border-[#2D313A] fill-gray-400 rounded-lg overflow-hidden shadow-xl mb-20" showInteractive={false} />
        
        <MiniMap 
            position="bottom-right"
            className="bg-[#181B21] border border-[#2D313A] rounded-lg overflow-hidden shadow-2xl mb-20" 
