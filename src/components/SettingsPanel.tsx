...
import { X, User, Bell, Shield, Sliders, Info, LogOut, Volume2, VolumeX, Moon, Check, Activity, Trash, Network, LayoutList, Calendar } from 'lucide-react';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen, onClose }) => {
  ...
  // Stores
  const { layoutPreference, setLayoutPreference, setGraph, clearHistory } = useNodesStore();
  ...
  const handleClearHistory = () => {
      if (confirm('Are you sure you want to clear your undo/redo history?')) {
          clearHistory();
          addToast({ type: 'success', title: 'History Cleared', message: 'Undo stack has been emptied.' });
      }
  };
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                ...
                {activeTab === 'general' && (
                    <div className="space-y-8 max-w-lg">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-white mb-1">Default View</h3>
                            <p className="text-sm text-gray-400 mb-4">Which mode should open on startup.</p>
                            <div className="flex gap-3">
                                <button
                                    onClick={() => updatePreferences({ defaultView: 'analysis' })}
                                    className={`flex-1 p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${
                                        preferences.defaultView === 'analysis' ? 'bg-[#FF4F5E]/20 border-[#FF4F5E] text-white' : 'bg-[#0F1115] border-[#2D313A] text-gray-400 hover:border-gray-500'
                                    }`}
                                >
                                    <Network size={18} />
                                ...
                                <button
                                    onClick={() => updatePreferences({ defaultView: 'management' })}
                                    className={`flex-1 p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${
                                        preferences.defaultView === 'management' ? 'bg-[#FF4F5E]/20 border-[#FF4F5E] text-white' : 'bg-[#0F1115] border-[#2D313A] text-gray-400 hover:border-gray-500'
                                    }`}
                                >
                                    <LayoutList size={18} />
                                ...
                                <button
                                    onClick={() => updatePreferences({ defaultView: 'workflow' })}
                                    className={`flex-1 p-3 rounded-xl border flex flex-col items-center gap-2 transition-all ${
                                        preferences.defaultView === 'workflow' ? 'bg-[#FF4F5E]/20 border-[#FF4F5E] text-white' : 'bg-[#0F1115] border-[#2D313A] text-gray-400 hover:border-gray-500'
                                    }`}
                                >
                                    <Calendar size={18} />
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                ...
                                <button 
                                    ...
                                    className="w-full p-4 border border-[#2D313A] bg-[#0F1115] rounded-xl text-left hover:bg-[#2D313A] transition-colors group flex items-center justify-between"
                                >
                                    <div>
                                        <span className="block text-sm font-bold text-gray-300 group-hover:text-white">Clear History</span>
                                        <span className="text-xs text-gray-500">Remove all undo/redo states.</span>
                                    </div>
                                    <Trash size={16} className="text-gray-500 group-hover:text-white" />
                                </button>
                                <button 
                ...
                {activeTab === 'profile' && (
                    <div className="space-y-8 max-w-lg">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-mono text-gray-500 mb-2">Bio</label>
                                <textarea 
                                    value={profile.bio || ''}
                                    onChange={(e) => updateProfile({ bio: e.target.value })}
                                    className="w-full bg-[#0F1115] border border-[#2D313A] rounded-lg px-4 py-3 text-sm text-white focus:border-[#FF4F5E] outline-none transition-colors h-24 resize-none"
                    ...
                    <div className="space-y-6 max-w-lg">
                        ...
                        <div className="flex items-center justify-between p-4 bg-[#0F1115] rounded-xl border border-[#2D313A]">
                            ...
                            <button 
                                ...
                                className={`w-10 h-5 rounded-full p-0.5 transition-colors ${preferences.reduceMotion ? 'bg-[#FF4F5E]' : 'bg-[#2D313A]'}`}
                            >
                                <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${preferences.reduceMotion ? 'translate-x-5' : 'translate-x-0'}`} />
                            </button>
                        </div>
                    </div>
                )}
